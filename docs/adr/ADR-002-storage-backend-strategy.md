...
5. [x] Ship `relio migrate --from <src> --to <dst>` (`relio/migrate.py` + CLI). Vectors are now **preserved** via `StorageBackend.iter_embeddings()` (SQLite unpacks sqlite-vec float32; Postgres parses the `pgvector` text form); records with a missing/mismatched-dim vector are re-embedded. `reembed=True` forces re-embedding (embedder/model change); `--no-embed` does a structured-only copy.
